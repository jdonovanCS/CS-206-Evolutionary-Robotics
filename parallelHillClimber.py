    ...
    def Record(self):
        ...
        for k in self.parents:
            if self.parents[k].fitness < best_value:
                ...
                best_key = k
        wandb.log({'gen': self.currentGeneration, 'best_fitness': best_value, 'num_hidden': 0, 'avg_fitness': np.mean([self.parents[k].fitness for k in self.parents]), 'avg_hidden': 0})
